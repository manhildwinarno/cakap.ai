...
const reportData = {
  score: 78,
  strengths: [
    "Structured answers using the STAR method effectively.",
    "Clear communication of technical concepts."
  ],
  improvements: [
    "Could provide more quantifiable metrics in the 'Result' phase.",
    "Need to elaborate on the specific 'Action' steps taken by you, rather than the team."
  ],
  questions: [
    {
      id: 1,
      question: "Tell me about a time you had to optimize the performance of a complex web application.",
      userAnswer: "In my last project, the dashboard was very slow. I used Chrome DevTools to find the bottleneck and then I implemented lazy loading for the images and memoized the heavy React components. It made the app much faster.",
      aiOptimized: "In my previous role, our main analytics dashboard was taking over 5 seconds to load (Situation). My task was to reduce this load time to under 2 seconds to improve user retention (Task). I profiled the application using Chrome DevTools and identified that large image payloads and unnecessary React re-renders were the main culprits. I implemented intersection observers for image lazy loading and applied `useMemo` and `React.memo` across our data grid components (Action). As a result, the initial load time dropped to 1.2 seconds, which directly contributed to a 15% increase in daily active users (Result).",
      needsWork: true,
    },
    {
      id: 2,
      question: "Describe a situation where you disagreed with a senior engineer on a technical approach.",
      userAnswer: "My senior wanted to use Redux for a simple form state. I thought it was overkill. I brought it up in a meeting and explained that local state or React Context would be faster to build and easier to maintain. He agreed and we went with Context.",
      aiOptimized: "On a recent project, a senior engineer proposed using Redux for managing the state of a multi-step form (Situation). I believed this was unnecessary overhead since the state didn't need to be accessed globally (Task). I scheduled a brief 1-on-1 to discuss the trade-offs, presenting a quick prototype using React Hook Form and Context API to demonstrate how it would reduce boilerplate by 40% (Action). He appreciated the proactive prototype, agreed with the simpler approach, and we successfully delivered the feature two days ahead of schedule (Result).",
      needsWork: false,
    }
  ]
};
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans pb-24 text-[#0F172A]">
      ...
      <main className="pt-28 px-6 lg:px-8 max-w-5xl mx-auto w-full">
        ...
        <div className="mb-10">
          ...
          <p className="text-[#3E4947] mt-2">Frontend Web Developer • Completed on Oct 24, 2024</p>
        </div>

        {/* ── Top Section: Bento Box Grid ── */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          
          {/* Card A: Readiness Score */}
            ...
            <div className="relative w-40 h-40 flex items-center justify-center mb-4">
              <svg className="w-full h-full transform -rotate-90 drop-shadow-sm" viewBox="0 0 100 100">
                ...
                <motion.circle 
                  ...
                  strokeLinecap="round" 
                  animate={{ strokeDashoffset: 264 - (264 * reportData.score) / 100 }}
                  transition={{ duration: 1.5, ease: "easeOut", delay: 0.2 }}
                />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                <span className="text-4xl font-bold text-[#0F172A] tracking-tighter">{reportData.score}<span className="text-xl text-[#64748B]">%</span></span>
              </div>
            </div>
            <h3 className="font-bold text-[#0F172A] text-lg">Overall Readiness</h3>
            <p className="text-sm text-[#3E4947] text-center mt-1">You're on the right track! Solid fundamentals.</p>
          </Card>
        ...
        {/* ── Lower Section: Detailed Feedback Stack ── */}
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-4">
            {reportData.questions.map((item, index) => {
              ...
              return (
                <Card key={item.id} className="border border-[#E2E8F0] shadow-sm bg-white overflow-hidden rounded-xl">
                  {/* Header / Trigger */}
                  <button 
                    ...
                    {isExpanded && (
                      ...
                      >
                        <div className="px-6 pb-6 pt-2 border-t border-[#F1F5F9] flex flex-col gap-6">
                          ...
                          {/* Block A: User's Raw Answer */}
                          <div className="bg-[#FFFBEB]/50 border border-[#FEF3C7] rounded-xl p-5">
                            <div className="flex items-center gap-2 mb-3">
                              <span className="bg-[#FEF3C7] text-[#D97706] text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full flex items-center gap-1.5">
                                <Star size={12} fill="currentColor" /> Needs STAR Method
                              </span>
                              <span className="text-xs font-semibold text-[#92400E] uppercase tracking-wider">Your Answer</span>
                            </div>
                            <p className="text-[#3F3F46] text-[15px] leading-relaxed">
                              "{item.userAnswer}"
                            </p>
                          </div>

                          {/* Block B: AI Optimized Answer */}
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-md border-t border-[#E2E8F0] shadow-[0_-4px_20px_rgb(0,0,0,0.03)] z-40 py-4">
        <div className="max-w-5xl mx-auto px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex w-full sm:w-auto items-center gap-4">
            <Button variant="outline" className="flex-1 sm:flex-none border-[#E2E8F0] text-[#3E4947] hover:bg-[#F8FAFC] gap-2 rounded-lg">
              <Download size={16} />
              Download PDF
            </Button>
            <Link href="/simulation" className="flex-1 sm:flex-none">
